...
const char *CATEGORIAS_STRINGS[CATEGORIAS] = {"1",
                                              "2",
                                              "3",
                                              "4",
                                              "5",
...
void limpiarConsola() { system(CLEAR); }

// Hacer una tirada de los dados, solo reemplazar por un numero random del 1 al
// 6 los dados que son 0
...
int hacerCambio(int dados[]) {
  ...
  if (cantDadosACambiar == DADOS) {
    ...
    for (i = 0; i < cantDadosACambiar; i++) {
      ...
      } while (dadoACambiar < 1 || dadoACambiar > DADOS ||
               !dados[dadoACambiar -
                      1]); // Si el jugador elije un numero que no esté entre
                           // [1, 5] o si elije un numero elegido anteriormente
...
void detectarOpciones(int dados[], int jugador[], int opciones[]) {
  ...
  for (i = 0; i < CARAS; i++) {
    ...
    if (jugador[6] == -1 &&
        ((contador[0] == 1 && contador[1] == 1 && contador[2] == 1 &&
          contador[3] == 1 && contador[4] == 1) ||
         (contador[1] == 1 && contador[2] == 1 && contador[3] == 1 &&
          contador[4] == 1 && contador[5] == 1))) {
...
void sumarPuntos(int opciones[], int jugador[]) {
  ...
  do {
    printf("\nElegi una opcion: ");
    scanf("%d", &decision);
  } while (decision <= 0 || decision > CATEGORIAS ||
           jugador[decision - 1] !=
               -1); // Si ingresa un int fuera del rango [1 , 11] o si elige una
                    // categoría que ya tiene tachada o anotada

  // Anotarse en la lista de puntos el valor de opciones en el indice elegido,
  // puede ser 0 o un numero mayor
...
void darOpcionesYSumar(int dados[], int jugador[]) {
  ...
  for (int i = 0; i < CATEGORIAS; i++) {
    if (opciones[i] != 0 &&
        jugador[i] == -1) { // Si se pueden sumar puntos en cierta categoría y
                            // el jugador no cuenta con puntos en esa categoria
      printf("%d) Anotarse: %d a %s\n", i + 1, opciones[i],
             CATEGORIAS_STRINGS[i]);
    } else if (jugador[i] ==
               -1) { // Si el jugador no cuenta con puntos en esa categoria
                     // (pero tampoco puede anotarse puntos)
...
// Recibe un array de enteros y devuelve suma sus elementos mayores o iguales a
// 0
int puntuacionesTotales(int puntuaciones[]) {
  ...
  for (int i = 0; i < CATEGORIAS; i++) {
    if (puntuaciones[i] >= 0)
      total += puntuaciones[i]; // Asegurarse que solo sume si el valor no es -1
  }
...
void imprimirPuntuaciones(int jugador1[], int jugador2[]) {
  ...
  printf("Total           |  %-3d  |  %-5d\n", puntuacionesTotales(jugador1),
         puntuacionesTotales(jugador2));
}

void turno(int jugador[], int turnoActual, int jugadorActual, int jugador1[],
           int jugador2[], double tiempoAcumulado) {
  ...
  while (i < RELANZADAS &&
         dadosACambiar) { // Si todavia no se relanzó todas las veces posibles y
                          // ademas hay dados para cambiar
    printf("Jugador: %d    |   Turno: %d   |   Tirada: %d   |   Tiempo "
           "Acumulado: %.0f segundos\n",
           jugadorActual, turnoActual, i + 1,
           tiempoAcumulado); // Imprimir info del juego
    mostrarDados(dados);
    dadosACambiar =
        hacerCambio(dados); // Invocar la funcion para cambiar los dados y
                            // actualizar la variable de dados a cambiar para
                            // salir del loop si es necesario
  ...
  printf("\n\nJugador: %d   |   Turno: %d\n", jugadorActual,
         turnoActual); // Imprimir info del juego
  printf("Tus dados son\n");
  mostrarDados(dados); // Mostrar los 5 dados elegidos
  darOpcionesYSumar(
      dados,
      jugador); // Imprimir las opciones posibles y sumarse o tacharse puntos
}
...
void evaluarResultadoIndividual(int jugador, int dificultad) {
  ...
  if (jugador >= dificultad) {
    ...
    printf("\n D: :c Perdiste :( T_T :C.\n\n Tu puntaje es %d y necesitabas %d "
           "para ganar\n",
           jugador, dificultad);
  }
}
...
void evaluarDesempenio(int puntosJugador, int numeroJugador) {
  ...
  if (puntosJugador < PUNTAJE_FACIL)
    veredicto = 1;
  else if (puntosJugador < PUNTAJE_MEDIA && puntosJugador >= PUNTAJE_FACIL)
    veredicto = 2;
  else if (puntosJugador < PUNTAJE_DIFICIL && puntosJugador >= PUNTAJE_MEDIA)
    veredicto = 3;
  else if (puntosJugador < PUNTAJE_MUY_DIFICIL &&
           puntosJugador >= PUNTAJE_DIFICIL)
    veredicto = 4;
  else if (puntosJugador < PUNTAJE_EXPERTO &&
           puntosJugador >= PUNTAJE_MUY_DIFICIL)
    veredicto = 5;
  else if (puntosJugador >= PUNTAJE_EXPERTO)
    veredicto = 6;
...
int elegirDificultad() {
  int dificultad;
  int puntajes[DIFICULTADES] = {PUNTAJE_FACIL,   PUNTAJE_MEDIA,
                                PUNTAJE_DIFICIL, PUNTAJE_MUY_DIFICIL,
                                PUNTAJE_EXPERTO, PUNTAJE_MAXIMO};

  printf("Elegi la dificultad [Puntuacion a vencer]\n\n1) Facil [%d]\n2) Media "
         "[%d]\n3) Dificil [%d]\n4) Muy dificil [%d]\n5) Experto [%d]\n6) "
         "Imposible [%d]\n",
         PUNTAJE_FACIL, PUNTAJE_MEDIA, PUNTAJE_DIFICIL, PUNTAJE_MUY_DIFICIL,
         PUNTAJE_EXPERTO, PUNTAJE_MAXIMO);
...
void unJugador() {
  ...
  // Inicializar los puntos del jugador 2 (en este caso a 0 para que cuente como
  // que no existe pero y no se queje la funcion de mostrar puntuaciones);
  ...
  // Inicializar reloj
  time_t inicioJugador, finJugador;
  double tiempoJugador = 0;

  for (i = 0; i < TURNOS; i++) {
    turnoActual = i + 1; // Calcular turno
    // Medir tiempo del jugador
    inicioJugador = time(NULL);
    turno(jugador1, turnoActual, 1, jugador1, jugador2,
          tiempoJugador); // En este caso, el tercer arg de la funcion turno
                          // siempre va a ser 1
    finJugador = time(NULL);
    tiempoJugador += (double)(finJugador - inicioJugador) /
                     CLOCKS_PER_SEC; // Calcular tiempo del turno
  }
...
void dosJugadores() {
  ...
  // Inicializar reloj
  time_t inicioJugador1, finJugador1;
  time_t inicioJugador2, finJugador2;

  for (int i = 0; i < TURNOS; i++) {
    int turnoActual = i + 1;

    // Medir tiempo del jugador 1
    inicioJugador1 = time(NULL); // Tomar tick del reloj
    turno(jugador1, turnoActual, 1, jugador1, jugador2, tiempoJugador1);
    finJugador1 = time(NULL); // Tomar tick del reloj
    tiempoJugador1 += (double)(finJugador1 - inicioJugador1) /
                      CLOCKS_PER_SEC; // Calcular el tiempo del turno

    // Medir tiempo del jugador 2
    inicioJugador2 = time(NULL); // Tomar tick del reloj
    turno(jugador2, turnoActual, 2, jugador1, jugador2, tiempoJugador2);
    finJugador2 = time(NULL); // Tomar tick del reloj
    tiempoJugador2 += (double)(finJugador2 - inicioJugador2) /
                      CLOCKS_PER_SEC; // Calcular el tiempo del turno
  }
  ...
  evaluarResultadoMultijugador(totalJugador1,
                               totalJugador2); // Mostrar quien gano

  double totalPartida =
      tiempoJugador1 + tiempoJugador2; // Calcular el tiempo total de la partida
  ...
  imprimirPuntuaciones(jugador1,
                       jugador2); // Mostrar la tabla de puntuaciones final
}
